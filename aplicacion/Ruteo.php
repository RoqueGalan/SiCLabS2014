...
class Ruteo {
    ...
    private function _CargarControladorExistente() {
        ...
        //si el controlador error entonces mandar al metodo error para tratarlo
        if (strtolower($this->_url[0]) == 'error') {
            $this->_error();
        }
    ...
    private function _LlamarMetodoDelControlador() {
        ...
        switch ($longitud) {
            case 5:
                ...
                $this->_controlador->{$metodo}($this->_url[2], $this->_url[3], $this->_url[4]);
                break;

            case 4:
                //Controller->Method(Param1, Param2)
                $this->_controlador->{$metodo}($this->_url[2], $this->_url[3]);
                break;

            case 3:
                //Controller->Method(Param1, Param2)
                $this->_controlador->{$metodo}($this->_url[2]);
                break;

            case 2:
                //Controller->Method(Param1, Param2)

                $this->_controlador->{$metodo}();
                break;
    ...
    private function _error() {
        ...
        // error/tipo/mensaje
        if (isset($this->_url[1]) && isset($this->_url[2])) {
            if (strtolower($this->_url[1]) == 'tipo') {
                $this->_controlador->tipo($this->_url[2]);
                exit;
            }
        }
